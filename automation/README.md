...
- `enable_backups` – Enable pgBackRest or WAL-G for an existing PostgreSQL cluster. When automatic backup configuration is enabled, the playbook provisions the required cloud backup storage. It then installs and configures the selected backup tool and updates `archive_command` in Patroni DCS. Set exactly one of `pgbackrest_install` or `wal_g_install` to `true`.
