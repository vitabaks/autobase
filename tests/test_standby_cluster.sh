...
export AWS_ACCESS_KEY_ID=$(grep -A2 "access_key:" $CONFIG_FILE | head -1 | awk -F'"' '{print $2}')
export AWS_SECRET_ACCESS_KEY=$(grep -A2 "secret_key:" $CONFIG_FILE | head -1 | awk -F'"' '{print $2}')
export AWS_DEFAULT_REGION=$(grep -A2 "region:" $CONFIG_FILE | head -1 | awk -F'"' '{print $2}')
